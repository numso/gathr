...
function grabAllInputs() {
  var promises = _.map(gathr.inputs, function (inp) {
    return inp.retrieve(inp.preformatter);
  });
...
function setupGathr(config) {
  var types = [
    { name: 'preformatters', fn: parsePreformatter },
...
function parseFile(filePath, opts) {
  ...
  var outputter = require(path);
  // if it's a node_module, require it the right way
  outputter = outputter(opts);
  if (!outputter.init) outputter.init = function () { return true; };
  return outputter;
}

function parsePreformatter(filePath, opts) {
  var preformatter = parseFile(filePath, opts);
  // validate preformatter
  return preformatter;
}

function parseInput(filePath, opts, gathr) {
  var input = parseFile(filePath, opts);
  if (opts.preformatter) {
    var pre = _.findWhere(gathr.preformatters, { id: opts.preformatter });
    if (!pre) throw Error ('No suitable parent for preformatter');
    input.preformatter = pre.format;
  } else {
    input.preformatter = JSON.parse;
  }
  return input;
}
